...
int main(void)
{
    ...
    float * host_buffer = (float*)malloc(4 * sizeof(float)*2);
    for (i = 0; i < 4; ++i) {
        host_buffer[i] = i;
    }

    // Create the device buffer
    cl_mem device_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, SAMPLE_SIZE * sizeof(float)*2, NULL, NULL);
    ...
    size_t n_work_groups = (SAMPLE_SIZE*2 + local_work_size + 1) / local_work_size;
